...
import { ChevronDown, ChevronUp, FileJson, Loader, Filter } from 'lucide-react';
import Layout from '../components/Layout';
import { auditLogData } from '../data/placeholderData';
...
export default function Audit() {
  ...
  const uniqueActions = Array.from(
    new Set((auditLogData as AuditEntry[]).map((entry) => entry.action))
  );

  // Filter entries
  const filteredEntries = (auditLogData as AuditEntry[]).filter((entry) => {
    const matchesAction = filterAction === 'all' || entry.action === filterAction;
    const matchesActor =
      searchActor === '' || entry.actor.toLowerCase().includes(searchActor.toLowerCase());
    ...
    <Layout>
      <div className="p-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-400">Track all actions and changes in the system</p>
        </div>
                  ...
                  <div key={entry.id}>
                    ...
                    >
                      <div className="grid grid-cols-12 gap-4 items-center text-sm">
                        ...
                        <div className="col-span-3 text-white font-medium">{entry.actor}</div>
...
function getActionSummary(entry: AuditEntry): string {
