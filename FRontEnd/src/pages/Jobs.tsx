...
export default function Jobs() {
  ...
  const [jobRows, setJobRows] = useState<Record<string, IngestionRow[]>>({});
  ...
  const fetchJobs = async () => {
    try {
      setIsLoading(true);
      const response = await apiFetch('/api/ingestion/jobs/', token);
  ...
  const toggleExpand = async (jobId: string) => {
    ...
    if (!jobRows[jobId]) {
      ...
      try {
        const response = await apiFetch(`/api/ingestion/jobs/${jobId}/rows/`, token);
        const data = await response.json();
        setJobRows((prev) => ({ ...prev, [jobId]: data }));
      } catch (err) {
        ...
        <div className="bg-gray-900 rounded-xl border border-gray-800 overflow-hidden">
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-24 px-4">
              ...
              <p className="text-gray-400 text-center max-w-sm">Upload a file to create your first import job.</p>
            </div>
          ) : (
            <>
              ...
              <div className="divide-y divide-gray-800">
                {jobs.map((job) => (
                  <div key={job.id}>
                    <button onClick={() => toggleExpand(job.id)} className="w-full px-6 py-4 hover:bg-gray-800/50 transition-colors">
                      <div className="grid grid-cols-12 gap-4 items-center text-sm">
                        <div className="col-span-1">
                          {expandedJobId === job.id ? <ChevronUp className="w-5 h-5 text-gray-400" /> : <ChevronDown className="w-5 h-5 text-gray-400" />}
                        </div>
                        <div className="col-span-3 text-white font-medium text-left">{job.raw_filename}</div>
                        <div className="col-span-2 text-gray-300 text-left">{job.source_type}</div>
                        <div className="col-span-2"><StatusBadge status={job.status} /></div>
                        <div className="col-span-1 text-center text-gray-300">{job.row_count.toLocaleString()}</div>
                        <div className="col-span-1 text-center">
                          {job.error_count > 0 ? <span className="text-red-400 font-medium">{job.error_count}</span> : <span className="text-gray-500">0</span>}
                        </div>
                        <div className="col-span-2 text-gray-400 text-left">{new Date(job.created_at).toLocaleString()}</div>
                      </div>
                    </button>
                    ...
                    {expandedJobId === job.id && (
                      <div className="bg-gray-800/30 px-6 py-4 border-t border-gray-800">
                        <div className="ml-4">
                          <h4 className="text-sm font-medium text-gray-400 mb-3">Ingestion Row Details</h4>
                          {loadingRows === job.id ? (
                                  ...
                                  <tbody className="divide-y divide-gray-800">
                                    {(jobRows[job.id] || []).map((row) => (
                                      <tr key={row.id} className="hover:bg-gray-800/30">
                                        <td className="px-4 py-3 text-sm text-gray-300">{row.row_index}</td>
                                        ...
                                        <td className="px-4 py-3 text-sm">
                                          {row.error_message ? <span className="text-red-400">{row.error_message}</span> : <span className="text-gray-500">-</span>}
                                        </td>
                                      </tr>
                                    ))}
                                  </tbody>
